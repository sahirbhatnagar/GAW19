...
data.clean <- function(filename, select.all=FALSE, nrows=10, ncols=10, col.alloc=100) {
    ...
    select.all=FALSE;ncols=960;nrows=100000; col.alloc=4500; filename="chr19-geno.csv"
    ...
    system(paste("cp plink.sh ",getwd(),"/chr",DT.all$chr[1],sep=""))
    setwd(paste(getwd(),"/chr",DT.snp$chr[1],sep=""))
    write.table(DT.final, file=paste(getwd(),"/chr",DT.snp$chr[1],"/chr",DT.snp$chr[1],".tped",sep=""), col.names=FALSE,
                row.names=FALSE, quote=FALSE)
...
data.clean("chr-19-geno.csv", select.all=TRUE, col.alloc=4000)
