...
# run this from GAW19/data/cleangeno/realpheno folder

# command line args: chromosome_number

# 5) Grab the SNP names to make the .MAP file
awk 'NR==1 {for (i=2;i<=NF;i++) print $i}' ~/share/sy/GAW19/data/cleangeno/chr$1/$1.temp.ped > chr$1/snp.id$1
...
join -12 -21 ~/share/sy/GAW19/data/PEDheader.fam ~/share/sy/GAW19/data/cleangeno/chr$1/$1.temp.ped > chr$1/$1.temp2.ped
...
# 9) Run PLINK to recode the alleles to 0/1/2, since FBAT only accepts this (note that this recodes the phenotypes to 2 which means affected. The --1 flag tells PLINK that control/case status is coded 0/1, and then --recode changes this to 1/2). this is for ALL individuals
plink.1.90betadev --ped chr$1/$1.temp3.ped --map chr$1/chr$1.map --missing-genotype X --recode 12 --1 --make-bed --remove ~/share/sy/GAW19/data/mztwins.txt --pheno ~/share/sy/GAW19/data/real.pheno --pheno-name hyper --missing-phenotype -9 --out chr$1/chr$1;
